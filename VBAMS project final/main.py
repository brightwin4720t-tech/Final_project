...
def send_user_notification(base_url: str, complaint_id: int, username: str, user_phone: str, user_email: str, mechanic_name: str, mechanic_phone: str):
    ...
    if user_email and smtp_user and "your_email" not in smtp_user and smtp_password and "your_app_password" not in smtp_password:
        try:
            msg = MIMEMultipart('alternative')
            msg['From'] = smtp_user
            msg['To'] = user_email
            msg['Subject'] = f"✅ Mechanic Assigned - Request #{complaint_id}"
            ...
            print(f"Notification SMTP sending failed: {e}")
    else:
        print("SMTP credentials not fully configured or user email empty. Email sending skipped.")
    ...
    print("="*50)
    print(f"EMAIL to [{user_email or 'N/A'}]:")
    print(f"   Subject: [SUCCESS] Mechanic Assigned - Request #{complaint_id}")
    print(f"   Mechanic Name: {mechanic_name}")
    print(f"   Mechanic Phone: {mechanic_phone}")
    print(f"   Track Link: {track_link}")
    print(f"   Status: {'SENT' if email_sent else 'SIMULATED (SMTP Credentials missing)'}")
    print("-" * 50)
    print(f"SMS to [{formatted_to}]:")
    print(f"   Content: {sms_body}")
    print(f"   Status: {'SENT' if sms_sent else 'SIMULATED (Twilio Credentials missing)'}")
    print("="*50 + "\n")
...
def send_reset_email_helper(email: str, role: str, reset_link: str):
    smtp_host = os.getenv("SMTP_HOST", "")
    smtp_port = os.getenv("SMTP_PORT", "587")
    smtp_user = os.getenv("SMTP_USER", "")
    smtp_password = os.getenv("SMTP_PASSWORD", "")
    
    if smtp_user and "your_email" not in smtp_user and smtp_password and "your_app_password" not in smtp_password:
        try:
            msg = MIMEMultipart('alternative')
            msg['From'] = smtp_user
            msg['To'] = email
            msg['Subject'] = "🔑 Reset Your BreakdownAssist Password"
            
            text_body = f"Hello,\n\nYou requested a password reset for your BreakdownAssist account.\nPlease click the link below to set a new password:\n\n{reset_link}\n\nThis link will expire in 1 hour.\nIf you did not request this reset, please ignore this email.\n\nBest regards,\nBreakdownAssist Team"
...
def handle_forgot_password(
    ...
    background_tasks.add_task(send_reset_email_helper, email, role, reset_link)
